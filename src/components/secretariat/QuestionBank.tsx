  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
           <h1 className="text-2xl font-bold text-text-primary font-display uppercase tracking-tight">Question Bank</h1>
           <p className="text-text-secondary text-sm font-medium">Manage assessment questions and knowledge repository.</p>
        </div>
        <div className="flex gap-4">
          <button 
            onClick={resetToDefaults}
            className="flex items-center gap-3 bg-white border border-brand-primary/10 text-text-muted px-6 py-3.5 rounded-xl text-xs font-bold uppercase tracking-wider shadow-sm hover:text-brand-primary hover:border-brand-primary/10 transition-all active:scale-95"
          >
          ...
          <button 
            ...
            className="flex items-center gap-3 bg-brand-secondary text-white px-6 py-3.5 rounded-xl text-xs font-bold uppercase tracking-wider shadow-xl shadow-brand-secondary/10 hover:bg-brand-secondary/90 transition-all active:scale-95"
          >
      ...
      <div className="flex bg-bg-elevated/50 p-1.5 rounded-2xl w-fit">
        <button 
          ...
          className={cn(
            ...
            activeTab === AssessmentType.PROFESSIONAL_TECHNOLOGIST ? "bg-white text-brand-primary shadow-sm" : "text-text-muted hover:text-text-primary"
          )}
        >
          Ts. Assessment (GT)
        </button>
        <button 
          ...
          className={cn(
            "px-6 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider transition-all",
            activeTab === AssessmentType.CERTIFIED_TECHNICIAN ? "bg-white text-brand-primary shadow-sm" : "text-text-muted hover:text-text-primary"
          )}
        >
          Tc. Assessment (QT)
        </button>
      ...
      <div className="bg-white rounded-3xl p-3 border border-brand-primary/10 shadow-sm flex flex-col md:flex-row items-center gap-4">
        <div className="relative flex-1 w-full">
           <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-text-muted/50" size={18} />
           <input 
             type="text" 
             placeholder="Search by keyword or act section..." 
             value={searchTerm}
             onChange={e => setSearchTerm(e.target.value)}
             className="w-full pl-16 pr-6 py-4 bg-bg-elevated/50 border-none rounded-2xl focus:ring-2 focus:ring-brand-primary/10 outline-none text-xs font-semibold tracking-wider transition-all placeholder:text-text-muted/30 text-text-primary"
           />
        </div>
        <div className="flex items-center gap-2 w-full md:w-auto px-4">
           ...
           <select 
             ...
             className="bg-transparent border-none text-xs font-bold uppercase tracking-wider outline-none text-text-secondary cursor-pointer"
           >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
         {filtered.map((q, idx) => (
           <div key={q.id} className="bg-white rounded-3xl p-8 border border-brand-primary/10 shadow-sm hover:shadow-xl hover:shadow-brand-primary/5 transition-all group flex flex-col">
              <div className="flex justify-between items-start mb-6">
                 <div className="flex gap-2">
                    <span className="px-3 py-1 bg-brand-primary/5 text-brand-primary text-[10px] font-bold uppercase rounded-lg tracking-wider">{q.category}</span>
                    <span className={cn(
              ...
              <h3 className="font-bold text-text-primary mb-6 leading-relaxed font-display text-lg flex-1 truncate-multiple">
                <span className="text-brand-primary/20 mr-2 text-2xl">“</span>
                {q.text}
              </h3>
              <div className="space-y-3">
                 {q.options.map((opt, oIdx) => (
                   <div key={oIdx} className={cn(
                     ...
                     q.correctAnswer === oIdx ? "bg-emerald-50 border-emerald-200 text-emerald-700 shadow-sm" : "bg-bg-elevated border-brand-primary/5 text-text-muted"
                   )}>
                      <div className={cn(
                        "w-6 h-6 rounded-lg flex items-center justify-center text-[10px] font-bold",
                        q.correctAnswer === oIdx ? "bg-emerald-600 text-white" : "bg-bg-elevated text-text-muted"
                      )}>
                        {String.fromCharCode(65 + oIdx)}
                      </div>
                      <span className="flex-1 truncate">{opt}</span>
                      {q.correctAnswer === oIdx && <CheckCircle size={16} className="text-emerald-600 shrink-0" />}
                   </div>
                 ))}
              </div>
           </div>
      ...
      {showAdd && (
        <div className="fixed inset-0 bg-brand-secondary/60 backdrop-blur-md z-50 flex items-center justify-center p-6 animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl overflow-hidden border border-brand-primary/10 flex flex-col max-h-[90vh]">
             <div className="p-10 border-b border-brand-primary/5 flex justify-between items-center bg-bg-elevated/50">
                <div>
                   <h3 className="text-2xl font-black text-text-primary font-display uppercase tracking-tight flex items-center gap-3">
                      <HelpCircle className="text-brand-primary" size={28} />
                      Knowledge Question
                   </h3>
                   <p className="text-text-muted text-[10px] font-black uppercase tracking-[0.2em] mt-1">Registry Data Collection Terminal</p>
                </div>
                <button 
                  onClick={() => {
                    setShowAdd(false);
                  ...
                  className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-text-muted hover:text-text-primary shadow-sm border border-brand-primary/10 transition-all hover:rotate-90"
                >
                  <X size={20} />
                </button>
             </div>
